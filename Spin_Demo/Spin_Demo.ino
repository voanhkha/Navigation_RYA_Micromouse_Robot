...
int WheelSpeed =  130;
#define LeftBaseSpeed 73
#define RightBaseSpeed 83
...
void setup() // run once before void loop
{
  ...
  reset();
//  Display(current_target, 8);
//  Display(next_target, 6);
//  Display(10,7);
...
void MoveNoLine(){
  ...
  while(flag_detect_line == 0){
    ...
    z_float = z*0.00875 - compass_offset;
...
void SpinLeftToLine(){
  analogWrite(LEFT_MOTOR, LeftBaseSpeed+20);
  analogWrite(RIGHT_MOTOR, 0);
  int spin_flag = 0;
  int time = millis();
  while(spin_flag == 0){
...
void SpinRightToLine(){
  analogWrite(LEFT_MOTOR, 0);
  analogWrite(RIGHT_MOTOR, RightBaseSpeed+20);
  int spin_flag = 0;
  int time = millis();
  while(spin_flag == 0){
    getIR();
...
void getIR()
{
  ...
  posi_error = LeftestIR*(-20)+LeftIR*(-10)+RightIR*10+RightestIR*20;
...
void SpinStepRight(){
  analogWrite(LEFT_MOTOR, 0);
  analogWrite(RIGHT_MOTOR, RightBaseSpeed+50);
  delay(10);
  analogWrite(RIGHT_MOTOR, 0);
}
...
void SpinStepLeft(){
  analogWrite(RIGHT_MOTOR, 0);
  analogWrite(LEFT_MOTOR, LeftBaseSpeed+50);
  delay(10);
  analogWrite(LEFT_MOTOR, 0);
}
